...
v99 n,k,tc=1,koita,mod=1000000007LL,a[mxi];
v99 dfs(v99 ind)
{
    if(ind>koita)return 0LL;
    v99 kot=dfs(ind+1);
    return (a[ind]*kot +a[ind]+kot)%mod;
}
...
void solve()
{
    ...
    {
        v99 kot=min(s[i],s[j])-'a'+1;
        v99 sum=((kot-1)*bigmod(k,koita-i,mod))%mod;
        a[i]=sum;
    }
    rep1(i,0,koita)cout<<a[i]<<whp;cout<<ses;
    v99 cr_h=(dfs(0LL)+is_pal(s))%mod;
